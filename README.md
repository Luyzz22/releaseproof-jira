...
- Dependency-Review SCRUM-80, Stand 2026-09-20: `npm audit --omit=dev --audit-level=high` meldet nach dem gezielten Bridge-Minor-Update und transitiven Security-Patches einschließlich `fast-uri@3.1.8` keine Befunde; das vollständige Audit meldet 0 High/Critical und zwei unveränderte Moderate-Einträge für `vitest` / `@vitest/mocker` ([GHSA-82fw-gwwq-j7x9](https://github.com/advisories/GHSA-82fw-gwwq-j7x9)). Die zusätzlich geprüften [Fast-URI-Upstream-Advisories](https://github.com/fastify/fast-uri/security/advisories) enthalten Befunde, die dieser npm-Audit-Lauf nicht erfasst; die betroffenen 3.x-Fälle sind im gewählten Patchstand korrigiert. Die Forge CLI sowie die direkten API-, KVS-, Resolver-, React-, TypeScript-, Vite- und Vitest-Versionen bleiben unverändert. Ein sauberer Audit-Stand ist kein Nachweis allgemeiner Sicherheit oder einer verifizierten Forge-Deployment-Kompatibilität. Abhängigkeitsketten, Reichweite und Entscheidungen stehen im [Learning Log](docs/learning-log.md).
